...
function CommentItem({
  ...
  const addReply = trpc.announcements.comment.useMutation({
    onSuccess: () => {
      setReplyText("");
      setReplying(false);
      onReplySuccess();
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2 mb-0.5">
            {comment.authorUsername ? (
              <Link href={`/profile/${comment.authorUsername}`} className="font-medium hover:underline truncate">
                {comment.authorName}
              </Link>
            ) : (
          ...
          <p className="leading-relaxed whitespace-pre-wrap">{renderContent(comment.content)}</p>
          {!disabled && (
            <button
              type="button"
        ...
        <div className="ml-8 space-y-2 border-l-2 border-border/60 pl-3">
          {replies.map(reply => (
            <div key={reply.id} className="flex gap-2 rounded-lg bg-muted/25 px-3 py-2 text-sm">
              ...
              />
              <div className="min-w-0 flex-1">
                <p className="leading-relaxed text-sm whitespace-pre-wrap">{renderContent(reply.content)}</p>
              </div>
            </div>
          ))}
        </div>
...
export function AnnouncementCommentSection({
  announcementId,
  disabled,
}: {
  announcementId: number;
  ...
  const { data: comments, isLoading } = trpc.announcements.comments.useQuery(
    { announcementId },
    { enabled: !disabled && announcementId > 0 }
  );

  const addComment = trpc.announcements.comment.useMutation({
    onSuccess: () => {
      setText("");
      utils.announcements.comments.invalidate({ announcementId });
  ...
  if (disabled) {
    return (
      <p className="text-sm text-muted-foreground text-center py-2">
        Comentários disponíveis após conectar o banco de dados.
      </p>
    );
  }
    ...
    <div className="mt-4 pt-4 border-t border-border space-y-4">
      ...
      <div className="pt-2 border-t border-border/50 space-y-3">
        ...
        <Button
          ...
          onClick={() => addComment.mutate({ announcementId, content: text.trim() })}
        >
